(function ($) {
  ...
  function sendCommand(cmd) {
    ...
    xhttp.open("GET", url + "/command?command=" + cmd, true);
  ...
  function sendData() {
    ...
    xhttp.open("GET", url + "/update?data=" + JSON.stringify(dataObj), true);
    xhttp.onerror = function () {
      console.log("** An error occurred during the transaction");
      if (pageNum == 0)
        switchOnline(false);
